pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                sshagent(['tomcat']) {
                    sh 'scp -o StrictHostKeyChecking=no target/demo.war ubuntu@3.235.191.176:/home/ubuntu/'
                    sh 'ssh ubuntu@3.235.191.176 "sudo mv /home/ubuntu/demo.war /opt/tomcat/webapps/"'
                }
            }
        }
    }
